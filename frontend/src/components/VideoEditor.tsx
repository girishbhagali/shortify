...
export default function VideoEditor({ videoUrl: videoUrlProp = null }: VideoEditorProps) {
  const state = useVideoEditor();
  const router = useRouter();
  const videoUrlQuery = videoUrlProp;
  ...
  useEffect(() => {
    ...
    if (videoUrlQuery) {
      const isBlob = videoUrlQuery.startsWith('blob:') || videoUrlQuery.startsWith('data:');
      let cleanUrl = videoUrlQuery;
      ...
      if (!isBlob) {
        ...
        cleanUrl = videoUrlQuery.includes('cacheBuster')
          ? videoUrlQuery
          : `${videoUrlQuery}${separator}cacheBuster=${Date.now()}`;
      }

      console.log("Setting store video URL to cleanUrl:", cleanUrl);
      state.setVideoUrl(cleanUrl);
      
      // Clean the URL bar so that refreshing the page starts fresh
      router.replace('/editor', { scroll: false });
    } else if (!state.videoUrl) {
      console.log("Setting store video URL to default fallback (blank)");
      state.setVideoUrl(""); // Start totally fresh with no video
    }
        ...
        <aside className="w-16 border-r border-[#27272a] bg-[#18181b] flex flex-col items-center py-5 gap-5 shrink-0">
          {[
            ...
            <button
              ...
              className={`p-3 rounded-xl flex flex-col items-center gap-1 transition-all relative group ${activeTab === tool.id ? 'bg-[#7c3aed]/15 text-[#a78bfa] border border-[#7c3aed]/40' : 'text-[#71717a] hover:text-[#e4e4e7] hover:bg-[#27272a]'
                }`}
              title={tool.label}
            >
        ...
        <main className="flex-1 bg-[#09090b] flex flex-col items-center justify-center p-8 relative overflow-hidden">
          <div className="relative aspect-video w-full max-w-4xl bg-[#18181b] rounded-2xl overflow-hidden shadow-2xl border border-[#27272a]">
            ...
            {state.videoUrl ? (
              ...
              <div className="w-full h-full flex flex-col gap-4 items-center justify-center text-[#71717a] bg-[#09090b]">
                <p>No video loaded</p>
                <input 
                  type="file" 
                  ...
                  onChange={(e) => {
                    ...
                    if (file) {
                      ...
                      state.setVideoUrl(objectUrl);
                    }
                  }}
                  className="hidden" 
                  id="video-upload" 
                />
                <label htmlFor="video-upload" className="bg-[#27272a] hover:bg-[#3f3f46] text-white px-4 py-2 rounded-lg cursor-pointer transition-colors shadow-sm text-sm font-bold flex items-center gap-2">
                  <MonitorPlay className="w-4 h-4" /> Import Local Video
                </label>
              </div>
            )}
            ...
            {state.crop !== 'Original' && (
              <div className="absolute inset-0 pointer-events-none flex items-center justify-center border-4 border-dashed border-[#8b5cf6]/20 bg-black/40">
                <div className={`border-2 border-dashed border-[#c084fc] bg-[#c084fc]/5 shadow-[0_0_50px_rgba(0,0,0,0.8)] relative flex items-center justify-center transition-all ${state.crop === '9:16' ? 'w-[31.6%] h-full' :
              ...
              <div className="grid grid-cols-2 gap-3">
                {['Original', '9:16', '1:1', '16:9'].map((ratio) => (
                  <button
                    ...
                    className={`py-4 rounded-xl border text-xs font-extrabold transition-all relative ${state.crop === ratio ? 'bg-[#7c3aed]/10 border-[#c084fc] text-white shadow-md' : 'bg-[#09090b] border-[#27272a] text-[#a1a1aa] hover:bg-[#27272a]'
                      }`}
                  >
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[0.5, 0.75, 1, 1.25, 1.5, 2].map((s) => (
                    <button
                      ...
                      className={`py-2 text-xs font-bold rounded-lg border transition-all ${state.speed === s ? 'bg-[#7c3aed] border-[#c084fc] text-white' : 'bg-[#09090b] border-[#27272a] text-[#a1a1aa] hover:bg-[#27272a]'
                        }`}
                    >
                      {s}x
                    </button>
              ...
              <div className="grid grid-cols-2 gap-2">
                {['None', 'Bright', 'Contrast', 'Vintage', 'B&W', 'Warm', 'Cool'].map((f) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all ${state.filters.name === f ? 'bg-[#7c3aed]/10 border-[#c084fc] text-white shadow-md' : 'bg-[#09090b] border-[#27272a] text-[#a1a1aa] hover:bg-[#27272a]'
                      }`}
                  >
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(['720p', '1080p', '4k'] as const).map((res) => (
                      <button
                        ...
                        className={`py-3 text-xs font-bold rounded-xl border transition-all ${exportResolution === res ? 'bg-[#7c3aed] border-[#c084fc] text-white shadow-md' : 'bg-[#09090b] border-[#27272a] text-[#a1a1aa] hover:bg-[#27272a]'
                          }`}
                      >
                        {res}
                      </button>
